...
/**
 * REST controller for managing {@link com.ingstic2.myapp.domain.Batiment}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BatimentResource {
    ...
    public List<Batiment> getAllBatiments(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Batiments");
        if (eagerload) {
            return batimentRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<Batiment> getBatiment(@PathVariable Long id) {
        ...
        Optional<Batiment> batiment = batimentRepository.findOneWithEagerRelationships(id);
