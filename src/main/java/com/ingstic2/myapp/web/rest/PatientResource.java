...
/**
 * REST controller for managing {@link com.ingstic2.myapp.domain.Patient}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PatientResource {
    ...
    public ResponseEntity<Patient> partialUpdatePatient(
        ...
        Optional<Patient> result = patientRepository
            ...
            .map(existingPatient -> {
                ...
                if (patient.getTabac() != null) {
                    existingPatient.setTabac(patient.getTabac());
                }
                if (patient.getAlcool() != null) {
                    existingPatient.setAlcool(patient.getAlcool());
                }
                if (patient.getSport() != null) {
                    existingPatient.setSport(patient.getSport());
                }
                if (patient.getCigarette() != null) {
                    existingPatient.setCigarette(patient.getCigarette());
                }
                if (patient.getContactParent() != null) {
                    existingPatient.setContactParent(patient.getContactParent());
                }
    ...
    public List<Patient> getAllPatients(@RequestParam(required = false) String filter) {
        if ("etudiant-is-null".equals(filter)) {
            log.debug("REST request to get all Patients where etudiant is null");
            return StreamSupport
                .stream(patientRepository.findAll().spliterator(), false)
                .filter(patient -> patient.getEtudiant() == null)
                .collect(Collectors.toList());
        }

        if ("personnel-is-null".equals(filter)) {
            log.debug("REST request to get all Patients where personnel is null");
            return StreamSupport
                .stream(patientRepository.findAll().spliterator(), false)
                .filter(patient -> patient.getPersonnel() == null)
                .collect(Collectors.toList());
        }
