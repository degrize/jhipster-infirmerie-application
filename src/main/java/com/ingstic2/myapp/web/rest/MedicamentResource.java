...
/**
 * REST controller for managing {@link com.ingstic2.myapp.domain.Medicament}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class MedicamentResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/medicaments/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Medicament> partialUpdateMedicament(
        ...
        Optional<Medicament> result = medicamentRepository
            ...
            .map(existingMedicament -> {
                if (medicament.getMedicament() != null) {
                    existingMedicament.setMedicament(medicament.getMedicament());
                }
    ...
    public List<Medicament> getAllMedicaments(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
    ...
    public ResponseEntity<Medicament> getMedicament(@PathVariable Long id) {
        ...
        Optional<Medicament> medicament = medicamentRepository.findOneWithEagerRelationships(id);
