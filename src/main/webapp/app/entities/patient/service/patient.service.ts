...
type RestOf<T extends IPatient | NewPatient> = Omit<T, 'dateNaissance'> & {
...
export class PatientService {
  ...
  protected convertDateFromClient<T extends IPatient | NewPatient | PartialUpdatePatient>(patient: T): RestOf<T> {
    return {
      ...patient,
      dateNaissance: patient.dateNaissance?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPatient: RestPatient): IPatient {
    return {
      ...restPatient,
      dateNaissance: restPatient.dateNaissance ? dayjs(restPatient.dateNaissance) : undefined,
    };
  }
