...
import { EntityArrayResponseType, ConstanteService } from '../service/constante.service';
...
export class ConstanteComponent implements OnInit {
  ...
  trackId = (_index: number, item: IConstante): number => this.constanteService.getConstanteIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.constantes = this.refineData(dataFromBody);
  }

  protected refineData(data: IConstante[]): IConstante[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IConstante[] | null): IConstante[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.constanteService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
