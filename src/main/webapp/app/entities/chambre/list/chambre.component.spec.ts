import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { ChambreService } from '../service/chambre.service';

import { ChambreComponent } from './chambre.component';

describe('Chambre Management Component', () => {
  let comp: ChambreComponent;
  let fixture: ComponentFixture<ChambreComponent>;
  let service: ChambreService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'chambre', component: ChambreComponent }]), HttpClientTestingModule],
      declarations: [ChambreComponent],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: {
            data: of({
              defaultSort: 'id,asc',
            }),
            queryParamMap: of(
              jest.requireActual('@angular/router').convertToParamMap({
                page: '1',
                size: '1',
                sort: 'id,desc',
              })
            ),
            snapshot: { queryParams: {} },
          },
        },
      ],
    })
      .overrideTemplate(ChambreComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(ChambreComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(ChambreService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.chambres?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to chambreService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getChambreIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getChambreIdentifier).toHaveBeenCalledWith(entity);
      expect(id).toBe(entity.id);
    });
  });
});
